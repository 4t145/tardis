...
async fn test_web_client() -> TardisResult<()> {
    ...
    let response = TardisFuns::web_client().get_to_str("https://www.baidu.com", vec![("User-Agent", "Tardis")]).await?;
    assert_eq!(response.code, StatusCode::OK.as_u16());
    assert!(response.body.unwrap().contains("baidu"));

    let response = TardisFuns::web_client().get_to_str("https://postman-echo.com/get", vec![("User-Agent", "Tardis")]).await?;
    assert_eq!(response.code, StatusCode::OK.as_u16());
    assert!(response.body.unwrap().contains("Tardis"));

    let response = TardisFuns::web_client().delete_to_void("https://postman-echo.com/delete", vec![("User-Agent", "Tardis")]).await?;
    assert_eq!(response.code, StatusCode::OK.as_u16());
